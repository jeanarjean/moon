...
function push_lead_to_usoft($contact_form, &$abort, $submission)
{
  ...
  $bodyPayload = [
    "contact" => [
      ...
      "addresses" => [
        [
          "type" => "Principal",
          "line" => $submission->get_posted_data('address-line1'),
          "city" => $submission->get_posted_data('city'),
          "state" => "QC",
          "country" => "CA",
  ...
  try {
    $headers = [
      ...
      "X-API-KEY: eoyqmpughn8uf54umktljyvjqhowa3" 
    ];
    $body = fetch("POST", "https://partner.plannit.io/api/jobs", $headers, json_encode($bodyPayload));

    error_log("PHP Debug: plannit response: {$body}");
    error_log("PHP Notice: added lead: {$prettyQuote}");
  } catch (Exception $e) {
    error_log("PHP Error: Failed to push lead to plannit {$prettyQuote}.\n {$e}");
  }
};
